    ...
    if (isShowLittle) {
      if (i % 5 === 0) {
        scaleArray.push(<line key={i + 1} max={beginKD} min={endKD} type={markName + '_line'}
                              x1={(stepSide ? (x - width) : (x + width))} y1={parseFloat(endY - (disPX * i)).toFixed(2)}
  ...
  for (let i = 0; i < wdPoints.length; i++) {
    ...
    for (let j = 0; j < wdPoint.length; j++) {
      ...
      if (phValue !== undefined && phValue !== "") {
        ...
        if (value !== phValue) {
          pointArray.push(drawCircle('wljw', {"x": xPx, "y": wljwY}, 4, "red", "物理降温:" + phValue + "°C", false));
        }
      }
    }
  }
  ...
  for (let i = 0; i < coor.length; i++) {
    ...
    for (let j in json) {
      ...
      if (ismbxlwd && mbY === xlY && xlY === wdY && gw) { //脉搏+心率+肛温[红圆+红圈+蓝圈]：两个圈在外围（红圈在最外层表示心率，篮圈在里层表示肛温）、一个圆在中间（红圆表示脉搏）
        ...
        mbY && pointArray.push(drawCircle("mb", {"x": j, "y": mbY}, 4, "red", "脉搏：" + json[j].mb.value + "次", true));//脉搏
        ...
        xlY && pointArray.push(drawCircle("xl", {"x": j, "y": xlY}, 4, "red", "心率：" + json[j].xl.value + "次", false));//心率
      ...
      function drawWd(point, j, wdY) {
        if (wdY && wdY < wdMax) {
          ...
          if (point.wd.type === "yw") {
            wdY && pointArray.push(drawCross("yw", {"x": j, "y": wdY}, 4, "blue", "腋温：" + point.wd.value + "°C"));//温度
...
function drawArrow(xy, length, type, color, title) {
  ...
  return (
    <svg key={xy.y + (type ? "up" : "down")}>
      <line x1={x} y1={y1} x2={x} y2={y2} style={{stroke: color, strokeWidth: 1.3}} markerEnd='url(#markerArrow)'/>
      <defs>
        <marker id='markerArrow' markerWidth='8' markerHeight='8' refX='4' refY='4.5' orient='auto'>
          <path d='M1.62,1.62 L1.62,7.92 L7.2,4.32 L1.62,1.62' style={{fill: color}} title={title ? title : ""}/>
        </marker>
      </defs>
    </svg>
  );
